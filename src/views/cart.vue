<template>
  <div class="cart">
    ...
    <div class="cart-content">
      ...
      >
        <div class="cart-info">
          <img :src="productD[item.id].image" />
          <span>{{ productD[item.id].name }}</span>
        </div>
        <div class="cart-price">￥ {{ productD[item.id].cost }}</div>
        ...
        <div class="cart-cost">
          ￥ {{ productD[item.id].cost * item.count }}
        </div>
    ...
    <div class="cart-footer" v-show="cartList.length">
      ...
      <div class="cart-footer-desc">
        <div class="cart-control-order" @click="back()">现在结算</div>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    costAll() {
      let cost = 0;
      this.cartList.forEach((item) => {
        cost += this.productD[item.id].cost * item.count;
      });
      return cost;
    },
  },
